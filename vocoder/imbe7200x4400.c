...
int mbe_decodeImbe4400Parms(char* imbe_d, mbe_parms* cur_mp, mbe_parms* prev_mp)
{
    ...
    if (b0 > 207) {
        if ((b0 >= 216) && (b0 <= 219)) {
#ifdef IMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: IMBE: Silence");
#endif
        }
        else {
#ifdef IMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: IMBE: Invalid fundamental frequency");
#endif
        }
        return (1);
    }
    ...
    if ((L > 56) || (L < 9)) {
#ifdef IMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: IMBE: invalid L: %i", L);
#endif
        return (1);
    }
...
#ifdef IMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: IMBE: b0:%i L:%i K:%i", b0, L, K);
#endif
    ...
    for (i = 6; i < 85; i++) {
        bb[*bo1][*bo2] = imbe_d[i];
#ifdef IMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: IMBE: bo1: %i, bo2: %i", *bo1, *bo2);
#endif
        bo1 += 2;
        bo2 += 2;
    }
    ...
    Gm[1] = B2[b2];
#ifdef IMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: IMBE: G1: %e, %s, %i", Gm[1], tmpstr, b2);
#endif

#ifdef IMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: IMBE: tmpstr: %s b2: %i g1: %e", tmpstr, b2, Gm[1]);
#endif
...
#ifdef IMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: IMBE: G%i: %e, %s, %i, ba1: %e, ba2: %e", i, Gm[i], tmpstr, bm, *ba1, *ba2);
#endif
        ba1 += 2;
        ba2 += 2;
    }
...
#ifdef IMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: IMBE: sum: %e ", sum);
#endif
        }
        Ri[i] = sum;
#ifdef IMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: IMBE: R%i: %e", i, Ri[i]);
#endif
    }
#ifdef IMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: IMBE: R1: %e", Ri[1]);
#endif
...
#ifdef IMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: IMBE: T1: %e", Tl[1]);
#endif
    ...
    for (l = 1; l <= cur_mp->L; l++) {
        ...
        intkl[l] = (int)(flokl[l]);
#ifdef IMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: IMBE: flokl: %e, intkl: %i", flokl[l], intkl[l]);
#endif
        // eq. 76
        deltal[l] = flokl[l] - (float)intkl[l];
#ifdef IMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: IMBE: deltal: %e", deltal[l]);
#endif
        // eq 77
...
#ifdef IMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: IMBE: Sum77: %e", Sum77);
#endif
...
#ifdef IMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: IMBE: rho: %e c1: %e c2: %e Sum77: %e T%i: %e log2M%i: %e M%i: %e", rho, c1, c2, Sum77, l, Tl[l], l, cur_mp->log2Ml[l], l, cur_mp->Ml[l]);
#endif
    }
