...
int mbe_decodeAmbe2400Parms(char* ambe_d, mbe_parms* cur_mp, mbe_parms* prev_mp)
{
    ...
    if ((b0 & 0x7E) == 0x7E) {
        ...
        if (b1 < 5) {
            LogWarning(LOG_HOST, "MBE: AMBE: index: %d, was <5, invalid!", b1);
            silence = 1;
        }
        else if ((b1 >= 5) && (b1 <= 122)) {
            LogWarning(LOG_HOST, "MBE: AMBE: index: %d, Single tone hz: %f", b1, (float)b1 * 31.25);
        }
        else if ((b1 > 122) && (b1 < 128)) {
            LogWarning(LOG_HOST, "MBE: AMBE: index: %d, was >122 and <128, invalid!", b1);
            silence = 1;
        }
        else if ((b1 >= 128) && (b1 <= 163)) {
            LogWarning(LOG_HOST, "MBE: AMBE: index: %d, Dual tone?", b1);
            // note: dual tone index is different on ambe(dstar) and ambe2+
        }
        else {
            LogWarning(LOG_HOST, "MBE: AMBE: index: %d, was >163, invalid!", b1);
            silence = 1;
        }

        if (silence == 1) {
#ifdef AMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: AMBE: Silence Frame");
#endif
...
#ifdef AMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: AMBE: Tone Frame");
#endif
        return (3);
    }
...
#ifdef AMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: AMBE: jl[%i]:%i Vl[%i]:%i", l, jl, l, cur_mp->Vl[l]);
#endif
    }

#ifdef AMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: AMBE: b0:%i w0:%f L:%i b1:%i", b0, cur_mp->w0, L, b1);
#endif
...
#ifdef AMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: AMBE: b2: %i, deltaGamma: %f gamma: %f gamma-1: %f", b2, deltaGamma, cur_mp->gamma, prev_mp->gamma);
#endif
...
#ifdef AMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: AMBE: b3: %i Gm[2]: %f Gm[3]: %f Gm[4]: %f b4: %i Gm[5]: %f Gm[6]: %f Gm[7]: %f Gm[8]: %f", b3, Gm[2], Gm[3], Gm[4], b4, Gm[5], Gm[6], Gm[7], Gm[8]);
#endif
    ...
    for (i = 1; i <= 8; i++) {
        ...
        Ri[i] = sum;
#ifdef AMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: AMBE: R%i: %f ", i, Ri[i]);
#endif
    }
...
#ifdef AMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: AMBE: Ji[1]: %i Ji[2]: %i Ji[3]: %i Ji[4]: %i", Ji[1], Ji[2], Ji[3], Ji[4]);
    LogDebug(LOG_HOST, "MBE: AMBE: b5: %i b6: %i b7: %i b8: %i", b5, b6, b7, b8);
#endif
...
#ifdef AMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: AMBE: C1,%i: %f ", k, Cik[1][k]);
#endif
        }
    }
...
#ifdef AMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: AMBE: C2,%i: %f ", k, Cik[2][k]);
#endif
        }
    }
...
#ifdef AMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: AMBE: C3,%i: %f ", k, Cik[3][k]);
#endif
        }
    }
...
#ifdef AMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: AMBE: C4,%i: %f ", k, Cik[4][k]);
#endif
        }
    }
...
#ifdef AMBE_DEBUG
                LogDebug(LOG_HOST, "MBE: AMBE: j: %i Cik[%i][%i]: %f ", j, i, k, Cik[i][k]);
#endif
                sum = sum + ((float)ak * Cik[i][k] * cosf((M_PI * (float)(k - 1) * ((float)j - (float)0.5)) / (float)ji));
            }
            Tl[l] = sum;
#ifdef AMBE_DEBUG
            LogDebug(LOG_HOST, "MBE: AMBE: Tl[%i]: %f", l, Tl[l]);
#endif
            l++;
        }
    }
    ...
    for (l = 1; l <= cur_mp->L; l++) {
        ...
        intkl[l] = (int)(flokl[l]);
#ifdef AMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: AMBE: flok%i: %f, intk%i: %i ", l, flokl[l], l, intkl[l]);
#endif
        // eq. 41
        deltal[l] = flokl[l] - (float)intkl[l];
#ifdef AMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: AMBE: delta%i: %f ", l, deltal[l]);
#endif
        // eq 43
...
#ifdef AMBE_DEBUG
    LogDebug(LOG_HOST, "MBE: AMBE: Sum43: %f", Sum43);
#endif
...
#ifdef AMBE_DEBUG
        LogDebug(LOG_HOST, "MBE: AMBE: flokl[%i]: %f, intkl[%i]: %i", l, flokl[l], l, intkl[l]);
        LogDebug(LOG_HOST, "MBE: AMBE: deltal[%i]: %f", l, deltal[l]);
        LogDebug(LOG_HOST, "MBE: AMBE: prev_mp->log2Ml[%i]: %f", l, prev_mp->log2Ml[intkl[l]]);
        LogDebug(LOG_HOST, "MBE: AMBE: BigGamma: %f c1: %f c2: %f Sum43: %f Tl[%i]: %f log2Ml[%i]: %f Ml[%i]: %f", BigGamma, c1, c2, Sum43, l, Tl[l], l, cur_mp->log2Ml[l], l, cur_mp->Ml[l]);
#endif
    }
